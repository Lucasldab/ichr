    ...
    for (i, file) in filtered.into_iter().enumerate() {
        ...
        set.spawn(async move {
            ...
            for url in &file.downloads {
                ...
                is_url_allowlisted(url, &file.path)?;
...
mod tests {
    ...
    fn test_allowlist_accepts_cdn_modrinth() {
        assert!(
            is_url_allowlisted("https://cdn.modrinth.com/data/foo.jar", "mods/foo.jar").is_ok(),
    ...
    fn test_allowlist_accepts_maven_fabricmc_net() {
        assert!(
            is_url_allowlisted("https://maven.fabricmc.net/foo/bar.jar", "mods/bar.jar").is_ok(),
    ...
    fn test_allowlist_rejects_attacker_com() {
        let err = is_url_allowlisted("http://attacker.com/backdoor.jar", "mods/backdoor.jar")
    ...
    fn test_allowlist_rejects_typo_modrinth() {
        let err = is_url_allowlisted("https://cdn.modrinth.co/foo.jar", "mods/foo.jar")
